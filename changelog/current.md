...
- feat(hiclaw-controller): add SandboxBackend (`HICLAW_WORKER_BACKEND=sandbox`) with OpenKruise Agent Sandbox plugin (`agents.kruise.io/v1alpha1`), supporting hibernate/resume lifecycle for Worker containers. The new backend uses a dynamic client to manage Sandbox CRs, maps Hibernated phase to StatusStopped for seamless reconciler integration, and falls back to Delete when hibernate capability is not available. Includes Watch integration for all three controllers (Worker, Team, Manager), Helm RBAC/env injection, and capability configuration via `HICLAW_SANDBOX_CAPABILITIES`.
