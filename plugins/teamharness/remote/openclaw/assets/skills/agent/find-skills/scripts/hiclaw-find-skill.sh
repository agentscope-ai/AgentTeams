...
find_broker_descriptor() {
    if [ -n "${TEAMHARNESS_CREDENTIAL_BROKER_DESCRIPTOR:-}" ] && [ -f "${TEAMHARNESS_CREDENTIAL_BROKER_DESCRIPTOR}" ]; then
        printf '%s\n' "${TEAMHARNESS_CREDENTIAL_BROKER_DESCRIPTOR}"
        return
    fi

    current="${SCRIPT_DIR}"
    while [ "${current}" != "/" ]; do
        candidate="${current}/.teamharness/credential-broker.json"
        if [ -f "${candidate}" ]; then
            printf '%s\n' "${candidate}"
            return
        fi
...
load_broker_skill_registry() {
    descriptor="$(find_broker_descriptor || true)"
    [ -n "${descriptor}" ] || return 0

    eval "$(
        "${TEAMHARNESS_NODE_BIN:-node}" "${SCRIPT_DIR}/load-broker-skill-registry.js" "${descriptor}"
    )"
}
