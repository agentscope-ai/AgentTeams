...
- **CoPaw local model provider URLs**: CoPaw provider bridging now maps `localhost` / `127.0.0.1` model base URLs to `host.docker.internal` while running in a container, so local OpenAI-compatible model checks and real Matrix chat calls use the same reachable host address.
