...
For ordinary Direct Reply in the current room/session, answer directly. Use the
`message` MCP tool only when a message must leave the current runtime
conversation.
...
Workers and remote members do not use the `message` MCP tool. They reply in the
current room/session, submit task results with `taskflow`, and let the Leader
handle cross-room or requester-route delivery.
