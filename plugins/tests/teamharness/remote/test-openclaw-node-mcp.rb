...
mcp_server = repo_root / "plugins/teamharness/remote/openclaw/node-mcp/server.js"
...
Dir.mktmpdir("teamharness-node-mcp-test-") do |tmp|
  ...
  Open3.popen3(
    ...
    "node",
    "server.js",
    { chdir: mcp_server.dirname.to_s }
  ) do |stdin, stdout, _stderr, wait_thread|
    begin
      init = rpc(stdin, stdout, 1, "initialize", {})
      assert(init.dig("serverInfo", "name") == "teamharness-openclaw", "initialize server name mismatch")
      ...
      assert(health["runtime"] == "openclaw", "health runtime must be openclaw")
