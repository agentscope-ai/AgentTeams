...
- **Always use task-management scripts to modify state.json** — never edit manually with jq. `manage-state.sh` handles task lifecycle fields; `check-progress-watchdog.sh` handles heartbeat watchdog snapshots
