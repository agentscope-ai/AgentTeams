...
QWENPAW_WORKER_CACHE_REF ?= $(QWENPAW_WORKER_IMAGE):buildcache
QWENPAW_WORKER_BUILDX_CACHE_ARGS ?= \
	--cache-from type=registry,ref=$(QWENPAW_WORKER_CACHE_REF) \
	--cache-to type=registry,ref=$(QWENPAW_WORKER_CACHE_REF),mode=max
...
# pull_request_target integration jobs load legacy hiclaw/* artifacts from the
# base workflow. Local builds still install the AgentTeams tags by default.
ifdef SKIP_BUILD
INSTALL_LOCAL_MANAGER        = $(LEGACY_LOCAL_MANAGER)
INSTALL_LOCAL_MANAGER_COPAW  = $(LEGACY_LOCAL_MANAGER_COPAW)
INSTALL_LOCAL_WORKER         = $(LEGACY_LOCAL_WORKER)
INSTALL_LOCAL_COPAW_WORKER   = $(LEGACY_LOCAL_COPAW_WORKER)
INSTALL_LOCAL_HERMES_WORKER  = $(LEGACY_LOCAL_HERMES_WORKER)
INSTALL_LOCAL_OPENHUMAN_WORKER = $(LOCAL_OPENHUMAN_WORKER)
INSTALL_LOCAL_QWENPAW_WORKER = $(LOCAL_QWENPAW_WORKER)
INSTALL_LOCAL_CONTROLLER     = $(LEGACY_LOCAL_CONTROLLER)
INSTALL_LOCAL_EMBEDDED       = $(LEGACY_LOCAL_EMBEDDED)
else
INSTALL_LOCAL_MANAGER        = $(LOCAL_MANAGER)
INSTALL_LOCAL_MANAGER_COPAW  = $(LOCAL_MANAGER_COPAW)
INSTALL_LOCAL_WORKER         = $(LOCAL_WORKER)
INSTALL_LOCAL_COPAW_WORKER   = $(LOCAL_COPAW_WORKER)
INSTALL_LOCAL_HERMES_WORKER  = $(LOCAL_HERMES_WORKER)
INSTALL_LOCAL_OPENHUMAN_WORKER = $(LOCAL_OPENHUMAN_WORKER)
INSTALL_LOCAL_QWENPAW_WORKER = $(LOCAL_QWENPAW_WORKER)
INSTALL_LOCAL_CONTROLLER     = $(LOCAL_CONTROLLER)
INSTALL_LOCAL_EMBEDDED       = $(LOCAL_EMBEDDED)
endif
...
ifndef SKIP_BUILD
	...
	AGENTTEAMS_NON_INTERACTIVE=1 AGENTTEAMS_VERSION=$(VERSION) AGENTTEAMS_MOUNT_SOCKET=1 \
		...
		AGENTTEAMS_INSTALL_MANAGER_IMAGE=$(INSTALL_LOCAL_MANAGER) \
		HICLAW_INSTALL_MANAGER_IMAGE=$(INSTALL_LOCAL_MANAGER) \
		AGENTTEAMS_INSTALL_WORKER_IMAGE=$(INSTALL_LOCAL_WORKER) \
		HICLAW_INSTALL_WORKER_IMAGE=$(INSTALL_LOCAL_WORKER) \
		AGENTTEAMS_INSTALL_COPAW_WORKER_IMAGE=$(INSTALL_LOCAL_COPAW_WORKER) \
		HICLAW_INSTALL_COPAW_WORKER_IMAGE=$(INSTALL_LOCAL_COPAW_WORKER) \
		AGENTTEAMS_INSTALL_HERMES_WORKER_IMAGE=$(INSTALL_LOCAL_HERMES_WORKER) \
		HICLAW_INSTALL_HERMES_WORKER_IMAGE=$(INSTALL_LOCAL_HERMES_WORKER) \
		AGENTTEAMS_INSTALL_OPENHUMAN_WORKER_IMAGE=$(INSTALL_LOCAL_OPENHUMAN_WORKER) \
		HICLAW_INSTALL_OPENHUMAN_WORKER_IMAGE=$(INSTALL_LOCAL_OPENHUMAN_WORKER) \
		AGENTTEAMS_INSTALL_QWENPAW_WORKER_IMAGE=$(INSTALL_LOCAL_QWENPAW_WORKER) \
		HICLAW_INSTALL_QWENPAW_WORKER_IMAGE=$(INSTALL_LOCAL_QWENPAW_WORKER) \
		AGENTTEAMS_INSTALL_CONTROLLER_IMAGE=$(INSTALL_LOCAL_CONTROLLER) \
		HICLAW_INSTALL_CONTROLLER_IMAGE=$(INSTALL_LOCAL_CONTROLLER) \
	...
	if [ -f "$$ENV_FILE" ]; then \
		...
		if [ -n "$$WORKSPACE_DIR" ] && [ -d "$$WORKSPACE_DIR" ]; then \
			...
			if [ "$$RUNTIME" = "copaw" ]; then \
				RM_IMAGE="$(INSTALL_LOCAL_MANAGER_COPAW)"; \
			else \
				RM_IMAGE="$(INSTALL_LOCAL_MANAGER)"; \
			fi; \
...
ifndef SKIP_BUILD
	...
	AGENTTEAMS_NON_INTERACTIVE=1 \
		...
		AGENTTEAMS_INSTALL_EMBEDDED_IMAGE=$(INSTALL_LOCAL_EMBEDDED) \
		HICLAW_INSTALL_EMBEDDED_IMAGE=$(INSTALL_LOCAL_EMBEDDED) \
		AGENTTEAMS_INSTALL_MANAGER_IMAGE=$(INSTALL_LOCAL_MANAGER) \
		HICLAW_INSTALL_MANAGER_IMAGE=$(INSTALL_LOCAL_MANAGER) \
		AGENTTEAMS_INSTALL_MANAGER_COPAW_IMAGE=$(INSTALL_LOCAL_MANAGER_COPAW) \
		HICLAW_INSTALL_MANAGER_COPAW_IMAGE=$(INSTALL_LOCAL_MANAGER_COPAW) \
		AGENTTEAMS_INSTALL_WORKER_IMAGE=$(INSTALL_LOCAL_WORKER) \
		HICLAW_INSTALL_WORKER_IMAGE=$(INSTALL_LOCAL_WORKER) \
		AGENTTEAMS_INSTALL_COPAW_WORKER_IMAGE=$(INSTALL_LOCAL_COPAW_WORKER) \
		HICLAW_INSTALL_COPAW_WORKER_IMAGE=$(INSTALL_LOCAL_COPAW_WORKER) \
...
help: ## Show this help
	...
	@echo "  AGENTTEAMS_LLM_API_KEY=sk-xxx AGENTTEAMS_DATA_DIR=~/hiclaw-data make install  # With external data dir"
