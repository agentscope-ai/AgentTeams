...
mc alias set "${AGENTTEAMS_STORAGE_ALIAS}" "${MINIO_S3_URL}" \
...
mkdir -p "${AGENTTEAMS_FS_ROOT}/hiclaw-config"
...
(
    while true; do
        sleep 10
        mc mirror "${AGENTTEAMS_STORAGE_PREFIX}/hiclaw-config/" "${AGENTTEAMS_FS_ROOT}/hiclaw-config/" --overwrite --remove --newer-than "15s" 2>/dev/null || true
    done
) &
