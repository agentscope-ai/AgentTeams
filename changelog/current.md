...
- **Manager Team creation guardrail**: The built-in Team Management skill now treats relationship questions as read-only and requires checking for an existing Team before running `hiclaw create team`, avoiding accidental duplicate Team creation when an admin asks about an existing Team.
