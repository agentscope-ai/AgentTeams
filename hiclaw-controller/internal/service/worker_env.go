...
func (b *WorkerEnvBuilder) Build(workerName string, prov *WorkerProvisionResult) map[string]string {
	...
	b.applyClusterDefaults(env, "AGENTTEAMS_")
	return env
}
...
func (b *WorkerEnvBuilder) applyClusterDefaults(env map[string]string, prefix string) {
	for k, v := range map[string]string{
		prefix + "MATRIX_DOMAIN":    b.defaults.MatrixDomain,
		prefix + "FS_ENDPOINT":      b.defaults.FSEndpoint,
		prefix + "FS_BUCKET":        b.defaults.FSBucket,
		prefix + "STORAGE_PREFIX":   b.defaults.StoragePrefix,
		prefix + "STORAGE_PROVIDER": b.defaults.StorageProvider,
		prefix + "CONTROLLER_URL":   b.defaults.ControllerURL,
		prefix + "AI_GATEWAY_URL":   b.defaults.AIGatewayURL,
		prefix + "MATRIX_URL":       b.defaults.MatrixURL,
	} {
		if v != "" {
			env[k] = v
		}
	...
	if b.defaults.MatrixDebug {
		env[prefix+"MATRIX_DEBUG"] = "1"
	}

	// CMS observability configuration
	if b.defaults.CMSTracesEnabled {
		env[prefix+"CMS_TRACES_ENABLED"] = "true"
	}
	if b.defaults.CMSMetricsEnabled {
		env[prefix+"CMS_METRICS_ENABLED"] = "true"
	}
	if b.defaults.CMSEndpoint != "" {
		env[prefix+"CMS_ENDPOINT"] = b.defaults.CMSEndpoint
	}
	if b.defaults.CMSLicenseKey != "" {
		env[prefix+"CMS_LICENSE_KEY"] = b.defaults.CMSLicenseKey
	}
	if b.defaults.CMSProject != "" {
		env[prefix+"CMS_PROJECT"] = b.defaults.CMSProject
	}
	if b.defaults.CMSWorkspace != "" {
		env[prefix+"CMS_WORKSPACE"] = b.defaults.CMSWorkspace
	}
