...
- **OpenClaw Worker sync loop**: Worker local-to-remote sync now ignores files that are excluded from upload when detecting changes, and no longer pushes `HEARTBEAT.md` back to storage, preventing repeated empty sync cycles when runtime files are touched.
