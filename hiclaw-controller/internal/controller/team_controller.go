...
func (r *TeamReconciler) reconcileTeamNormal(ctx context.Context, t *v1beta1.Team) (reconcile.Result, error) {
	...
	for _, wn := range workerRuntimeNames {
		var roomID string
		if ms := t.Status.MemberByName(wn); ms != nil {
			roomID = ms.RoomID
		}
		teamWorkerEntries = append(teamWorkerEntries, service.TeamWorkerEntry{Name: wn, RoomID: roomID})
	}
...
func (r *TeamReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	// Watch Worker CRs whose status changes (for the decoupled path).
	// When a referenced Worker's status changes, the owning Team is enqueued
	// via the spec.workerMembers.name field indexer.
	bldr = bldr.Watches(
		&v1beta1.Worker{},
