...
- **Runtime-switch MCP contract**: Reject `--mcp-servers` during Worker runtime changes because MCP authorization must be applied separately after the replacement container converges. ([#1053](https://github.com/agentscope-ai/AgentTeams/pull/1053))
...
- **运行时切换 MCP 契约**：Worker 切换运行时时拒绝 `--mcp-servers`，因为 MCP 授权必须在替换容器收敛后单独应用。([#1053](https://github.com/agentscope-ai/AgentTeams/pull/1053))
