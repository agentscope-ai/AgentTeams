...
- **CoPaw filesync delete action + ignore rules + ghost-file mirror cleanup**: Add a new `filesync(action="delete")` shared-path API that removes files and directories from both MinIO and the local workspace. Introduce `.agentteamsignore` (legacy alias `.hiclawignore`) at the worker sync root — gitignore-style patterns in that file are honored by the periodic `push_local` uploader so transient tool artifacts (caches, per-project outputs, `.env` files) never leak to MinIO. Add `--remove` to directory `mc mirror` operations (explicit `pull`/`push` of shared directories) so a local file delete that was already pushed to MinIO does not get restored on the next pull, closing the previous "ghost file" cycle. ([5741852](https://github.com/agentscope-ai/AgentTeams/commit/5741852))
