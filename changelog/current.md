...
- **test(14) deterministic git-collab setup**: `tests/test-14-git-collab.sh` now pre-creates alice, bob, and charlie with `hiclaw apply worker` before sending the task, while leaving project-room creation in the Manager's normal project-management workflow. This removes Worker provisioning timing from the LLM control loop without bypassing project coordination.
