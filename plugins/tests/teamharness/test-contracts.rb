...
assert(!(plugin_root / "skills/team/organization/SKILL.md").exist?, "organization skill must not ship in qwenpaw TeamHarness skills")
expected_skills = {
  "agent" => {
  ...
  "team" => {
    ...
    "alibabacloud-schedulerx-ai-ops" => %w[leader]
  }
}
...
expected_skills.each do |group, expected|
  ...
  entries.each do |entry|
    ...
    expected_name = if group == "agent" || entry.fetch("id") == "alibabacloud-schedulerx-ai-ops"
      entry.fetch("id")
    else
      "teamharness-#{entry.fetch("id")}"
    end
