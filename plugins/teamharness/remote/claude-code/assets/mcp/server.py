...
def _credential_broker_descriptor_path() -> Path | None:
    explicit = os.getenv("TEAMHARNESS_CREDENTIAL_BROKER_DESCRIPTOR", "").strip()
    if explicit:
        return Path(explicit).expanduser()
    start = Path(__file__).resolve()
    for base in [start.parent, *start.parents]:
        candidate = base / ".teamharness" / "credential-broker.json"
        if candidate.exists():
            return candidate
    return None


def _credential_broker_get(path: str) -> dict[str, Any] | None:
    descriptor_path = _credential_broker_descriptor_path()
    if descriptor_path is None or not descriptor_path.exists():
        return None
    try:
        descriptor = json.loads(descriptor_path.read_text(encoding="utf-8"))
        endpoint = str(descriptor.get("endpoint") or "").rstrip("/")
        token_file = Path(str(descriptor.get("tokenFile") or "")).expanduser()
        token = token_file.read_text(encoding="utf-8").strip()
    except (OSError, json.JSONDecodeError) as exc:
        raise CredentialBrokerUnavailable(f"credential broker descriptor is invalid: {exc}") from exc
    if not endpoint or not token:
        raise CredentialBrokerUnavailable("credential broker descriptor missing endpoint or token file")
    request = urllib.request.Request(
        f"{endpoint}{path}",
        headers={
    ...
    try:
        ...
        raise CredentialBrokerUnavailable(f"credential broker error: HTTP {exc.code}: {body}") from exc
    except (OSError, TimeoutError, urllib.error.URLError, json.JSONDecodeError) as exc:
        raise CredentialBrokerUnavailable(f"credential broker unavailable: {exc}") from exc
    if not isinstance(data, dict):
        raise CredentialBrokerUnavailable("credential broker response must be a JSON object")
    return data


def _credential_broker_runtime_context() -> dict[str, Any] | None:
    try:
        return _credential_broker_get("/v1/runtime/context")
    except CredentialBrokerUnavailable:
        return None


def _credential_broker_storage() -> dict[str, Any] | None:
    try:
        return _credential_broker_get("/v1/credentials/storage")
    except CredentialBrokerUnavailable as exc:
        raise ValueError(str(exc)) from exc


def _matrix_credentials(tool: str) -> tuple[str, str, dict[str, Any]]:
    try:
        broker = _credential_broker_get("/v1/credentials/matrix")
    except CredentialBrokerUnavailable as exc:
        raise ValueError(str(exc)) from exc
    if broker is not None:
        homeserver = str(broker.get("homeserver") or "").rstrip("/")
        token = str(broker.get("accessToken") or "")
        if not homeserver or not token:
            raise ValueError("credential broker matrix credentials missing homeserver or accessToken")
        return homeserver, token, broker
...
def _message(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        homeserver, token, _credentials = _matrix_credentials("message")
    except ValueError as exc:
        return {"ok": False, "tool": "message", "error": str(exc)}
...
def _matrix_env(tool: str) -> tuple[str, str]:
    homeserver, token, _credentials = _matrix_credentials(tool)
    return homeserver, token
...
def _matrix_user_id() -> str:
    try:
        ...
        broker = None
    if broker is not None:
        user_id = str(broker.get("userId") or "").strip()
        if user_id:
            return user_id
...
def _default_shared_prefix() -> str:
    ...
    context = _credential_broker_runtime_context()
    storage_context = context.get("storage") if isinstance(context, dict) else None
    if isinstance(storage_context, dict):
        prefix = str(storage_context.get("sharedPrefix") or "").strip()
        if prefix:
            return prefix.strip("/")
...
def _default_global_shared_prefix() -> str:
    ...
    storage_context = context.get("storage") if isinstance(context, dict) else None
    if isinstance(storage_context, dict):
        prefix = str(storage_context.get("globalSharedPrefix") or "").strip()
        if prefix:
            return prefix.strip("/")
    storage = _section(_load_runtime_config(), "storage")
...
def _filesync_file_backend(
    ...
    root = _storage_object_root(credentials)
    remote_key = remote.strip("/")
    remote_path = root / remote_key
    if action == "list":
    ...
    if action == "stat":
        ...
        if not exists and is_directory:
            exists = any((root / remote_key.rstrip("/")).rglob("*")) if (root / remote_key.rstrip("/")).exists() else False
        ...
        if is_directory:
            ...
            if base.exists():
                for path in base.rglob("*"):
                    if path.is_file():
                        target = local / path.relative_to(base)
...
def _oss_bucket_from_credentials(credentials: dict[str, Any]) -> Any:
    try:
        import oss2  # type: ignore[import-not-found]
    except ImportError as exc:
        raise RuntimeError("oss2 is required for broker-backed filesync") from exc
    auth = oss2.StsAuth(
        str(credentials.get("accessKeyId") or ""),
        str(credentials.get("accessKeySecret") or ""),
        str(credentials.get("securityToken") or ""),
    )
    endpoint = str(credentials.get("endpoint") or "")
    bucket = str(credentials.get("bucket") or "")
    try:
        return _oss_bucket(oss2, auth, endpoint, bucket)
    except Exception:
        fallback_endpoint = _oss_public_endpoint_fallback(endpoint)
        if not fallback_endpoint:
            raise
        return _oss_bucket(oss2, auth, fallback_endpoint, bucket)


def _oss_public_endpoint_fallback(endpoint: str) -> str | None:
    ...
    if "oss-" not in host or not host.endswith(suffix):
        return None
    public_host = host[: -len(suffix)] + ".aliyuncs.com"
    netloc = public_host
...
def _oss_bucket(oss2_module: Any, auth: Any, endpoint: str, bucket: str) -> Any:
    try:
        return oss2_module.Bucket(auth, endpoint, bucket, connect_timeout=5)
    except TypeError:
        return oss2_module.Bucket(auth, endpoint, bucket)


def _filesync_oss_backend(
    action: str,
    local: Path,
    remote: str,
    is_directory: bool,
    ...
    if urllib.parse.urlparse(endpoint).scheme == "file":
        return _filesync_file_backend(action, local, remote, is_directory, exclude, credentials)

    bucket = _oss_bucket_from_credentials(credentials)
    remote_key = remote.strip("/")
    if action == "list":
        prefix = remote_key.rstrip("/") + "/" if is_directory else remote_key
        try:
            import oss2  # type: ignore[import-not-found]
            entries = [obj.key for obj in oss2.ObjectIterator(bucket, prefix=prefix)]
        except Exception as exc:
            raise RuntimeError(f"OSS list failed: {exc}") from exc
        return {"entries": entries}
    if action == "stat":
        try:
            if is_directory:
                import oss2  # type: ignore[import-not-found]
                exists = False
                for _obj in oss2.ObjectIterator(bucket, prefix=remote_key.rstrip("/") + "/"):
                    exists = True
                    break
            else:
                bucket.head_object(remote_key)
                exists = True
        except Exception:
            exists = False
        return {"exists": exists}
    if action == "pull":
        if is_directory:
            ...
            for obj in oss2.ObjectIterator(bucket, prefix=prefix):
                ...
                target.parent.mkdir(parents=True, exist_ok=True)
                target.write_bytes(bucket.get_object(obj.key).read())
        else:
            local.parent.mkdir(parents=True, exist_ok=True)
            local.write_bytes(bucket.get_object(remote_key).read())
        return {}
    if is_directory:
        if not local.exists():
            raise FileNotFoundError(f"local path not found: {local}")
        ...
        for path in local.rglob("*"):
            ...
            if _should_exclude(relative, exclude):
                continue
            bucket.put_object_from_file(prefix + relative, str(path))
    else:
        if not local.exists():
            raise FileNotFoundError(f"local path not found: {local}")
        bucket.put_object_from_file(remote_key, str(local))
    return {}
...
def _filesync(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        storage_credentials = _credential_broker_storage()
    except ValueError as exc:
        return {
            "ok": False,
            "tool": "filesync",
    ...
    if storage_credentials is not None:
        try:
            result = _filesync_oss_backend(action, local, remote, is_directory, exclude, storage_credentials)
        ...
        base["backend"] = "oss-sdk"
        if action == "list":
            base["entries"] = result.get("entries", [])
        if action == "stat":
            base["exists"] = bool(result.get("exists"))
        return base
