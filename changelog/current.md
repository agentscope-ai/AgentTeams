...
- **Human Worker access**: Human reconciliation now keeps standalone Worker `groupAllowExtra` entries in sync with `spec.accessibleWorkers`, so Human room access also grants runtime message permission for CoPaw and other allowlist-enforcing workers.
