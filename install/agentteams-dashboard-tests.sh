...
if [ ! -f "${MAKEFILE}" ]; then
    ...
    if grep -q '^DASHBOARD_VERSION ?= v1\.2\.4\.9$' "${MAKEFILE}"; then
        pass "Makefile dashboard default version = v1.2.4.9"
    else
        fail "Makefile dashboard default version is not v1.2.4.9"
    fi
fi
