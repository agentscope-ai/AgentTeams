...
func TestLifecycleWorkerStatusStandaloneNoTeamInfo(t *testing.T) {
	scheme := newLifecycleTestScheme(t)
	worker := &v1beta1.Worker{
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/workers/standalone/status", nil)
	req.SetPathValue("name", "standalone")
	rec := httptest.NewRecorder()
	...
	if resp.Team != "" {
		t.Errorf("expected Team empty for standalone worker, got %q", resp.Team)
	}
	if resp.Role != "" {
		t.Errorf("expected Role empty for standalone worker, got %q", resp.Role)
	}
}
