"""MinIO file sync shared across HiClaw worker runtimes.
...
File Sync Design Principle:
  ...
  Worker-managed (Worker read-write, push to MinIO):
    AGENTS.md, SOUL.md, <runtime_home_dir>/sessions/, memory/, etc.

  Local -> Remote (push_loop): change-triggered push of Worker-managed content.
  Remote -> Local (sync_loop pull_all): on-demand plus fallback periodic pull.

  The ``runtime_home_dir`` constructor parameter (default: ``.harness``) controls
  which hidden directory belongs to this runtime.  Pass ``.hermes`` for hermes,
  ``.harness`` for harness, etc.
"""
...
class FileSync:
    ...
    def push_local(self, since: float = 0) -> list[str]:
        ...
        _RUNTIME_DERIVED_FILES = {"config.json", ".env"}
        ...
        for path in local_dir.rglob("*"):
            ...
            if any(p in self._exclude_dirs for p in rel.parts):
                continue
            if rel.suffix in {".lock", ".db-journal", ".db-wal", ".db-shm"}:
                continue
            if (
                len(rel.parts) == 2
    ...
    while True:
        ...
        try:
            ...
            pushed = await asyncio.get_event_loop().run_in_executor(
                None, sync.push_local, last_push_time
            )
            last_push_time = now
            if pushed:
