...
if [ "${AGENTTEAMS_RUNTIME}" = "aliyun" ] || [ "${AGENTTEAMS_RUNTIME}" = "k8s" ]; then
    ...
    if [ "${AGENTTEAMS_RUNTIME}" = "k8s" ]; then
        ...
        # AGENTTEAMS_MANAGER_PASSWORD is optional in AppService mode (token pre-injected).
    else
...
if [ "${AGENTTEAMS_RUNTIME}" = "k8s" ]; then
    AGENTTEAMS_FS="/root/hiclaw-fs"
    mkdir -p "${AGENTTEAMS_FS}/shared" "${AGENTTEAMS_FS}/agents" "${AGENTTEAMS_FS}/hiclaw-config"
    log "Configuring mc alias for cluster MinIO (${AGENTTEAMS_STORAGE_ALIAS})..."
    mc alias set "${AGENTTEAMS_STORAGE_ALIAS}" "${AGENTTEAMS_FS_ENDPOINT}" "${AGENTTEAMS_FS_ACCESS_KEY}" "${AGENTTEAMS_FS_SECRET_KEY}"
...
if [ "${CMS_TRACES_ENABLED}" = "true" ]; then
    ...
    if [ ! -f "${CMS_PLUGIN_MANIFEST}" ]; then
        ...
        if [ "${_missing}" = "0" ]; then
            CMS_SERVICE_NAME="${AGENTTEAMS_CMS_SERVICE_NAME:-hiclaw-manager}"
...
if [ "${AGENTTEAMS_RUNTIME}" = "k8s" ]; then
    ...
    (
        while true; do
            ...
            mc mirror "${AGENTTEAMS_STORAGE_PREFIX}/shared/" /root/hiclaw-fs/shared/ --overwrite --newer-than "1m" 2>/dev/null || true
            mc mirror "${AGENTTEAMS_STORAGE_PREFIX}/agents/" /root/hiclaw-fs/agents/ --overwrite --newer-than "1m" 2>/dev/null || true
            mc mirror "${AGENTTEAMS_STORAGE_PREFIX}/hiclaw-config/" /root/hiclaw-fs/hiclaw-config/ --overwrite --newer-than "1m" 2>/dev/null || true
            mc cp "${AGENTTEAMS_STORAGE_PREFIX}/manager/openclaw.json" /root/manager-workspace/openclaw.json 2>/dev/null || true
        done
    ) &
