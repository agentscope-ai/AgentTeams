...
不要把市场右上角的**上传技能**当作 Worker 分发入口：该按钮只维护 Dashboard 的集中式市场。上传后还需从目标 Skill 所在行点击**分发到 Worker**。Worker 分发完成后，应在 Worker 详情的“已分发技能”中检查结果；Dashboard 会尝试休眠、唤醒 Worker，失败时新文件最长约 5 分钟内通过周期同步被发现。
