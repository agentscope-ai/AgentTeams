...
- **预发布自动化**: Release 工作流支持 SemVer 预发布 Tag，将 GitHub beta Release 标记为 prerelease，确保 beta / RC Tag 不更新稳定版镜像的 `latest`，预发布版本跳过 CoPaw PyPI 自动发布，固定 CoPaw 1.0.2 以保证 beta 镜像可复现，保持 Quick Start 默认安装最新稳定版、beta 仅允许显式选择，暂缓自动发布 QwenPaw 镜像，并修复集成 CI 对改名后本地 OpenClaw 基础镜像的引用。
